...
if w1:
    train_ids = [i for i in train_ids if 'w1' in i]
    label_ids = [i for i in label_ids if 'w1' in i]
else:
    train_ids = [i for i in train_ids if 'w2' in i]
    label_ids = [i for i in label_ids if 'w2' in i]
print(len(train_ids))
print(len(label_ids))
...
def read_train_data(IMG_WIDTH=256,IMG_HEIGHT=256,IMG_CHANNELS=1):
    ...
    Y_train = np.zeros((len(label_ids), IMG_HEIGHT, IMG_WIDTH, 1), dtype=np.bool)
